...
## There are tests for each part, but the tests are NOT exhaustive, because you may each have different tweets, etc. 
## Make sure you check the data you get -- print it out, check the types, look in the database browser, try out your SQL queries!
...
#(e.g. "umsi" or "umich" or "Lin_Manuel" or "ColleenAtUMSI") and returns data that represents at least 20 tweets from that user's timeline.
...
def get_user_tweets(twitter_handle):
	...
	if unique_identifier in CACHE_DICTION:
		...
		twitter_results = api.user_timeline(screen_name = twitter_handle) #count = 200 possibly?
